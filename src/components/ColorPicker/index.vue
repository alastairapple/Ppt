...
// Every time a non-preset color is selected, it needs to be added to the recent colors list
...
// Open eye dropper
// Check if the environment supports the native eye dropper, if so, use the native dropper, otherwise use the custom dropper
...
const browserEyeDropper = () => {
  message.success('Press ESC to close the eye dropper', { duration: 0 })
...
// Custom eye dropper based on Canvas
const customEyeDropper = () => {
  ...
  toCanvas(targetRef, { filter, fontEmbedCSS: '', width, height, canvasWidth: width, canvasHeight: height, pixelRatio: 1 }).then(canvasRef => {
    ...
    message.error('Eye dropper initialization failed')
    document.body.removeChild(maskRef)
  })
}
</script>
