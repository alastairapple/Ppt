...
export default () => {
  ...
  // Export pptist file (special .pptist suffix file)
  ...
  // Format HTML string into pptxgenjs required format
  // Core idea: tile the HTML string by style, each piece inherits ancestor style, block elements need line breaks.
  ...
  // Format SVG path information into pptxgenjs required format
  ...
  // Determine if it is a Base64 image address
  const isBase64Image = (url: string) => {
    const regex = /^data:image\/[^;]+;base64,/
    return url.match(regex) !== null
  }

  // Determine if it is an SVG image address
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          if (el.special) {
            ...
            if (svgRef.clientWidth < 1 || svgRef.clientHeight < 1) continue // Temporary fix (abnormal data from importing PPTX file)
