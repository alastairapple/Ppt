...
export default () => {
  ...
  /**
   * Add specified element data (a set)
    ...
    const newSlides = slides.map(slide => {
      ...
      for (const element of slide.elements) {
        ...
        if (element.link && element.link.type === 'slide') {
          ...
          // If the target slide is included in the slides to be added, replace the relevant binding
          if (slideIdMap[element.link.target]) {
            element.link.target = slideIdMap[element.link.target]
          }
          // If the target slide is not included in the slides to be added, delete the slide jump binding for this element
