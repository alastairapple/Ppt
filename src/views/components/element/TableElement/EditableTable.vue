...
// Calculate the column width and total width of each column in the table
...
// Clear the selection status of all cells
// Cells should also be cleared when the table is in a non-editable state
...
// Position of the operation node for dragging column width
...
// Collection of invalid cell positions (positions of merged cells)
const cells = computed(() => props.data)
const { hideCells } = useHideCells(cells)

// Current selected cell collection
...
// Current active cell: when there is only one selected cell, this cell is the active cell
const activedCell = computed(() => {
  if (selectedCells.value.length > 1) return null
  return selectedCells.value[0]
})

// Set selected cell status (mouse click or drag selection)
...
// Determine if a position is an invalid cell (position occupied by a merge)
...
// If there are cells to the right of the current row, move focus to the right
// If there are no cells to the right (already at the end of the row), and there is a next row, move focus to the beginning of the next row
// If there are no cells to the right (already at the end of the row), and there is no next row (already at the last row), create a new row and move focus to the beginning of the next row
const tabActiveCell = () => {
  ...
  // Automatically focus the text after moving focus
  focusActiveCell()
}
...
// Insert data from Excel, automatically supplement rows/columns if the table does not have enough
...
// Get effective cells (exclude merged cells)
...
// Check if rows and columns can be deleted: the number of effective rows/columns is greater than 1
...
// Check if merging or splitting is possible
// Multiple selections are required for merging
// Single selection and the selected cell is a merged cell are required for splitting
