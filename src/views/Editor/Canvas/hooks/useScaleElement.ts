...
/**
 * Calculates the positions of the eight resize handles of an element after rotation.
...
/**
 * Gets the position of the opposite handle relative to a given handle.
 * For example, 'top' corresponds to 'bottom', and 'top-left' corresponds to 'bottom-right'.
 * @param direction The current resize handle being operated.
 * @param points The positions of the eight resize handles after rotation.
 */
...
export default (
  ...
  const scaleElement = (e: MouseEvent | TouchEvent, element: Exclude<PPTElement, PPTLineElement>, command: OperateResizeHandlers) => {
    ...
    // Minimum size limit for elements
    ...
    // When scaling a rotated element, a base point concept is introduced.
    // The point opposite to the current resize handle becomes the base point.
    // For example, when dragging the bottom-right handle, the top-left is the base point.
    // The top-left point remains unchanged, and other points' positions are adjusted to achieve the desired effect.
    ...
    // For unrotated elements, alignment snapping is enabled during scaling.
    // Alignment lines are collected here.
    // This includes alignment positions (top, bottom, left, right edges) of other elements on the canvas,
    // excluding the target element. Lines and rotated elements do not participate in alignment.
    else {
      ...
      // Top, bottom, horizontal center, left, right, and vertical center of the canvas viewport
    ...
    // Alignment snapping function
    // Compares the collected alignment lines with the current position and size data of the target element.
    // If the difference is within the set range, automatic scaling correction is applied.
    // Horizontal and vertical directions are calculated separately.
    const alignedAdsorption = (currentX: number | null, currentY: number | null): { offsetX: number; offsetY: number } => {
    ...
    const handleMousemove = (e: MouseEvent | TouchEvent) => {
      ...
      // If the element is rotated, the scaling distance (distance moved after mouse down) needs to be recalculated based on the element's rotation angle.
      if (elRotate) {
        ...
        // Lock aspect ratio (only triggers for corner handles, not side handles)
        // The scaling distance in the horizontal direction is used as the basis to calculate the scaling distance in the vertical direction, maintaining the same scaling ratio.
        if (fixedRatio) {
        ...
        // Calculate the scaled size and position of the element separately based on different operation points.
        // Important Note:
        // The position calculated here needs to be corrected later because scaling a rotated element actually changes the position of the element's base point (although visually the base point appears stationary, it's the combined result of [rotation] + [movement]).
        // However, the size calculated here does not require re-correction because the scaling distance was already recalculated, effectively correcting the size.
        ...
        // Get the base point coordinates of the current element, compare them with the base point coordinates from the initial state, and calculate the difference for element position correction.
      ...
      // If the element is not rotated, simply calculate the new position and size as usual, without complex corrections.
      // Additional processing is required for alignment snapping.
      // Aspect ratio locking logic is the same as above, no further explanation needed.
      else {
  ...
  const scaleMultiElement = (e: MouseEvent, range: MultiSelectRange, command: OperateResizeHandlers) => {
    ...
    document.onmousemove = e => {
      ...
      // Lock aspect ratio, logic is the same as above
      ...
      // The ratio of the current element's width and height to the overall width and height of all selected elements
      ...
      // Based on the ratios calculated above, modify the position and size of all selected elements
      elementList.value = elementList.value.map(el => {
