<template>
  <div 
    class="canvas" 
    ref="canvasRef"
    @wheel="handleMousewheelCanvas"
    @mousedown="handleClickBlankArea"
  ...
  >
    <ElementCreateSelection
      ...
      @created="insertElementFromCreateSelection"
    />
    <ShapeCreateCanvas
      v-if="creatingCustomShape"
      @created="insertCustomShape"
    />
    <div 
...
// When the component is rendered, if there is element focus, it needs to be cleared.
// This situation occurs when: with element focus entered presentation mode, and then exited, the original focus needs to be cleared (possibly because the page has switched).
onMounted(() => {
...
// Click on the blank area of the canvas: clear the focused element, set the canvas focus, clear the text selection, clear the format painter state
...
// Double-click on the blank area to insert text
const handleDblClick = (e: MouseEvent) => {
  ...
  createTextElement({
    left,
    top,
    width: 200 / canvasScale.value, // Divide by canvasScale to maintain the same width as the element creation by click selection
    height: 0,
  })
}

// Clear the format painter state when the canvas is unmounted
onUnmounted(() => {
  if (textFormatPainter.value) mainStore.setTextFormatPainter(null)
})

// Remove editor area focus from the canvas
...
const handleMousewheelCanvas = (e: WheelEvent) => {
  ...
  // When Ctrl key is pressed: scale the canvas
...
// Insert element from area drawn by mouse
