  ...
  <div class="rich-text-base">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-model:value="AIPopoverVisible" style="width: 25%;">
        <template #content>
          <PopoverMenuItem center @click="execAI('美化改写')">Beautify</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('扩写丰富')">Expand</PopoverMenuItem>
          <PopoverMenuItem center @click="execAI('精简提炼')">Condense</PopoverMenuItem>
        </template>
        <CheckboxButton
          first
          style="width: 100%;"
    ...
    >
      ...
      <RadioButton value="justify" v-tooltip="'Align Justify'" style="flex: 1;"><IconAlignTextBoth /></RadioButton>
    </RadioGroup>
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error('No text content to execute on')
