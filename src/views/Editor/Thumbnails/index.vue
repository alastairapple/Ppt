...
watch(() => slideIndex.value, () => {
  ...
  // Check if the current page thumbnail is in the viewport, if not, scroll to the corresponding position
  nextTick(() => {
...
const handleClickSlideThumbnail = (e: MouseEvent, index: number) => {
  ...
  // Press Ctrl key, click to select slides, clicking an already selected slide again will deselect it
  // If the deselected slide is the current active slide, select the first of the other selected slides as the current active slide
  ...
  // Press Shift key, select all slides in the range
...
// Set thumbnail toolbar focus state (only when focused can the shortcut keys of this part take effect)
