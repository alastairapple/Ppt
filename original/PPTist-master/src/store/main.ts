...
export const useMainStore = defineStore('main', {
  state: (): MainState => ({
    activeElementIdList: [], // List of selected element IDs, includes handleElementId
    handleElementId: '', // ID of the element being operated on
    activeGroupElementId: '', // ID of the element within a group that can be independently operated on
    hiddenElementIdList: [], // List of hidden element IDs
    canvasPercentage: 90, // Visible area percentage of the canvas
    canvasScale: 1, // Canvas zoom scale (based on width {{slidesStore.viewportSize}} pixels)
    canvasDragged: false, // Canvas is being dragged
    thumbnailsFocus: false, // Focus on the thumbnail area on the left
    ...
    creatingElement: null, // Information about the element being inserted, elements that need to be inserted by drawing (text, shapes, lines)
    creatingCustomShape: false, // Drawing a freeform polygon
    toolbarState: ToolbarStates.SLIDE_DESIGN, // State of the toolbar on the right
    ...
    selectedSlidesIndex: [], // List of currently selected slide indices
    dialogForExport: '', // Export panel
    databaseId, // ID of the current application's indexedDB database
    ...
    showNotesPanel: false, // Open remarks panel
    showMarkupPanel: false, // Open type annotation panel
    showAIPPTDialog: false, // Open AI PPT creation window
  }),
