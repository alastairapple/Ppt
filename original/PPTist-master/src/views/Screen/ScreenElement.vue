...
// Determine if the element needs to wait for the entrance animation: elements that need to wait for the entrance animation need to be hidden first
const needWaitAnimation = computed(() => {
  ...
  // The element has not been animated
  if (elementIndexInAnimation === -1) return false

  // If the element has already executed the animation, it does not need to be hidden
  // Specifically: if the last executed animation is an entrance, it clearly does not need to be hidden; if the last executed animation is an exit, because the end state of the exit animation is preserved, it also does not need to be additionally hidden
  if (elementIndexInAnimation < props.animationIndex) return false

  // If the element has not been animated, get the first animation to be executed
  // If the first animation to be executed is an entrance, it needs to be hidden, otherwise it does not need to be hidden
