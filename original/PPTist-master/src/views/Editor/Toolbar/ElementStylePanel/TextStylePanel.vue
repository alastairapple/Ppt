<template>
  <div class="text-style-panel">
    <div class="preset-style">
      <div class="preset-style-item" v-for="item in presetStyles" :key="item.label" :style="item.style" @click="emitBatchRichTextCommand(item.cmd)">{{ item.label }}</div>
    </div>
...
// Note: There is an unknown bug. If the text box height increases after bolding, the canvas visible area positioning will be incorrect.
// Therefore, when executing preset style commands, place the bold command as early as possible to avoid issues caused by font size increase followed by bolding.
const presetStyles = [
  {
    label: 'Large Title',
    style: {
...
// Emit rich text setting command (batch)
