...
export default (canvasRef: ShallowRef<HTMLElement | null>) => {
  ...
  // Initialize the position of the canvas viewport
  ...
  // Update the position of the canvas viewport
  ...
  // When the viewport scales or the aspect ratio changes, reset/update the viewport position
  ...
  // When the canvas drag state changes (resets), reset the viewport position
  watch(canvasDragged, () => {
    if (!canvasDragged.value) initViewportPosition()
  })

  // Styles for the viewport's position and size
  ...
  // Observe changes in canvas size and reset the viewport position
