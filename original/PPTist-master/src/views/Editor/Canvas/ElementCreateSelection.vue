...
const createSelection = (e: MouseEvent) => {
  ...
  document.onmousemove = e => {
    ...
    // When Ctrl or Shift key is held down:
    // For non-line elements, lock the aspect ratio. For line elements, lock the horizontal or vertical direction.
    if (ctrlOrShiftKeyActive.value) {
      const moveX = currentPageX - startPageX
      const moveY = currentPageY - startPageY

      // Horizontal and vertical drag distances. The larger drag distance will be used as the basis for calculating the other direction's data.
      const absX = Math.abs(moveX)
      const absY = Math.abs(moveY)

      if (creatingElement.value.type === 'shape') {

        // Determine if it's an opposite drag: dragging from top-left to bottom-right is a positive operation; all other cases are opposite operations.
...
// Path data for drawing lines (only used when the element type being created is a line)
...
// Calculate the position and size of the element when created based on the start and end positions of the generation range
