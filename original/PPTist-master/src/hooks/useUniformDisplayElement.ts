...
export default () => {
  ...
  const uniformHorizontalDisplay = () => {
    ...
    // Separately get ordinary elements and group element collections, and record the range of each item
    ...
    // Combine the ordinary element and group element collections, then sort each item by position (from left to right)
    ...
    // Calculate the target position of each element sequentially according to the position order
    // The elements in the first item are the starting point and do not need to be calculated
    // Starting from the second item, the position of each item should be: previous item's position + previous item's width + interval
    // Note that the position (pos) calculated here is not the final left value of the element, but the minimum value of the target position range (the left value of the element after rotation is not equal to the minimum value of the range)
    ...
    // Calculate the final target left value of the elements based on the target position
    // For rotated elements, the offset of the left before and after rotation needs to be calculated for correction
  ...
  // Uniform vertical display (logic similar to uniform horizontal display method)
