...
export default () => {
  ...
  /**
   * Move slide focus
   * @param command Move slide focus command: move up, move down
   */
  ...
  // Attempt to decrypt clipboard slide data and add it as the next slide (paste)
  const pasteSlide = () => {
  ...
  // Create a blank slide and add it as the next slide
  const createSlide = () => {
    const emptySlide: Slide = {
      id: nanoid(10),
      elements: [],
  ...
  // Copy the current slide and paste it as the next slide
  const copyAndPasteSlide = () => {
    const slide = JSON.parse(JSON.stringify(currentSlide.value))
    addSlidesFromData([slide])
  }

  // Delete the current slide; if all slides are to be deleted, reset slides
  ...
  // Copy and then delete the current slide (cut)
  // Since the copy operation will cause the multi-select state to disappear, the IDs of the pages to be deleted need to be cached in advance
  const cutSlide = () => {
  ...
  // Synchronize data when dragging to adjust slide order
